    ...
    partial class BnulkMatrix
    {
        ...
        方法：
            ...
            * tqli(ref Vector d, ref Vector e, ref Matrix z) 计算特征值和特征向量。
              ...
              输出：d是特征值，e被破坏，z每一列是特征向量。
              另：如果输入z是单位矩阵的话，输出z的第k列是与d[k]相对应的规范特征向量。
        代码来源：
            * c++数值计算(第二版)
        ----------------------------------------------------  类注释  结束----------------------------------------------------
        */
        ...
        public static void tred2(ref BnulkMatrix a, ref BnulkVec d, ref BnulkVec e)
        {
            int l, k, j, i;
        ...
        /// <param name="z">输入：单位矩阵；输出：三对角短阵的特征向量。</param>
        /// <param name="z">输入：tred2输出的矩阵；输出：tred2已约化的矩阵的特征向量。</param>
        public static void tqli(ref BnulkVec d, ref BnulkVec e, ref BnulkMatrix z)
        {
            int m, l, iter, i, k;
