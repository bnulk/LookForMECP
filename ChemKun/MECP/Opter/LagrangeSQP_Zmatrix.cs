...
{
    partial class LagrangeSQP_Zmatrix
    {
        public LagrangeSQP_Zmatrix(Data_Input data_Input, ref Data_MECP data_MECP)
        {
            //初始化
            try
            {
        ...
        private void InitialWorkMatrix_Zmatrix(Data_Input data_Input, Data_MECP data_MECP, ref WorkMatrix_ZMatrix workMatrix_ZMatrix)
        {
            ...
            //定义和SQP相关的变量
            workMatrix_ZMatrix.B_Z = new double[3 * workMatrix_ZMatrix.N - 6, 3 * workMatrix_ZMatrix.N - 6];
            ...
            workMatrix_ZMatrix.tao = data_Input.mecpData.sqp_tao;
        ...
        private void GetNewX(Data_Input data_Input, ref Data_MECP data_MECP)
        {
            ...
            try
            {
                ...
                workMatrix_ZMatrix.MatrixG1, workMatrix_ZMatrix.MatrixG2, workMatrix_ZMatrix.MatrixH1, workMatrix_ZMatrix.MatrixH2, workMatrix_ZMatrix.tao,
                ref workMatrix_ZMatrix.F_Z, ref workMatrix_ZMatrix.Omiga_Z);
            }
            catch
            {
                Output.WriteOutput.m_Result.Append("ChemKun.MECP.Opter.LagrangeSQP_Zmatrix.CalFZAndOmiga_Z Error" + "\n");
                Console.WriteLine("ChemKun.MECP.Opter.LagrangeSQP_Zmatrix.CalFZAndOmiga_Z Error" + "\n");
            }
            //调用高斯-约当函数求方程的解
            try
            {
                CalGaussJordan(workMatrix_ZMatrix.N, workMatrix_ZMatrix.F_Z, workMatrix_ZMatrix.Omiga_Z, ref workMatrix_ZMatrix.DetParams_Z, ref workMatrix_ZMatrix.tmpOmiga_Z);
            }
            catch
            {
            ...
            try
            {
                UpdateX(data_Input, workMatrix_ZMatrix.DetParams_Z, workMatrix_ZMatrix.MatrixG1, workMatrix_ZMatrix.MatrixG2, workMatrix_ZMatrix.tao, ref data_MECP);
            }
            catch
            {
        ...
        public void CalFZAndOmiga_Z(double Labuta, int N, double Energy1, double Energy2, double[] MatrixG1, double[] MatrixG2, double[,] MatrixH1, double[,] MatrixH2, double tao, ref double[] F_Z, ref double[,] Omiga_Z)
        {
            //填充B_Z矩阵
            double[,] detOmiga_Z = new double[3 * N - 6, 3 * N - 6];
            double[] detG = new double[3 * N - 6];
            //计算detG
            for(int i = 0; i < 3 * N - 6; i++)
            {
                detG[i] = MatrixG2[i] - MatrixG1[i];
            }
            //计算detOmiga_Z
            for (int i = 0; i < 3 * N - 6; i++)
            {
                ...
                {
                    detOmiga_Z[i, j] = detG[i] * detG[j] / 2 / tao;
                }
            }
            for (int i = 0; i < 3 * N - 6; i++)
            {
                F_Z[i] = Convert.ToDouble(MatrixG1[i]);
            }
            F_Z[3 * N - 6] = Energy2 - Energy1;
            //F_Z[3 * N - 6] = Energy1 - Energy2;
            for (int i = 0; i < 3 * N - 6; i++)
            {
                for (int j = 0; j < 3 * N - 6; j++)
                {
                    Omiga_Z[i, j] = (1 - Labuta) * MatrixH1[i, j] + Labuta * MatrixH2[i, j] + detOmiga_Z[i, j];
                }
            }
            //填充梯度部分
            for (int i = 0; i < 3 * N - 6; i++)
            ...
            for (int i = 0; i < 3 * N - 6; i++)
            {
                Omiga_Z[3 * N - 6, i] = MatrixG2[i] - MatrixG1[i];
        ...
        //更新结果文件
        public void UpdateX(Data_Input data_Input, double[] DetParams_Z, double[] MatrixG1, double[] MatrixG2, double tao, ref Data_MECP data_MECP)
        {
            //给Lambda赋值
            double detLambda = 0.0;
            for (int i=0; i< 3 * workMatrix_ZMatrix.N - 6; i++)
            {
                detLambda += (MatrixG2[i] - MatrixG1[i]) * DetParams_Z[i];
            }
            detLambda = detLambda / 2 / tao;
            data_MECP.functionData.Lambda = DetParams_Z[3 * workMatrix_ZMatrix.N - 6] - detLambda;
            //更新I
            data_MECP.I++;
            //更新x
